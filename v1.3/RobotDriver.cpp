...
bool SchrittMotor::driveStepper(float pos){
    if(pos<=limit&&pos>=0&&position<=limit&&position>=0){
        Serial.println("Moving y to "+pos);
...
void Roboterarm::drive(float x, float y, float z, bool e){
    ...
    (*calculateAngles)(angles, x, y, z, shoulderHeight, xUpperLimit);
    ...
    if(ellbowChange>0) float ratio = shoulderChange/ellbowChange;
    else{
